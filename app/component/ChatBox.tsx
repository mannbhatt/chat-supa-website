...
const ChatBox = ({ session, selectedUser }: ChatBoxProps) => {
  ...
  useEffect(() => {
    ...
    roomOne.on("presence", { event: "sync" }, () => {
      ...
      //console.log("Online Users:", formattedPresence);
    });
  
    roomOne.on("presence", { event: "join" }, ({ key, newPresences }) => {
      //console.log("User Joined:", key, newPresences);
      setOnlineUsers((prev) => ({ ...prev, [key]: true }));
    });
  
   
    roomOne.on("presence", { event: "leave" }, ({ key, leftPresences }) => {
  ...
  const sendMessage = async () => {
    ...
    const { data, error } = await supabase.from("messages").insert([newMessageData]);
    ...
    <div className=" bg-white rounded-xl">
          ...
          {selectedUser ? (
            ...
            <div className="flex items-center">
              ...
              <p className="text-base text-gray-500">{onlineUsers[selectedUser.id] ? "online" : "offline"}</p>
              </span>
              </div> 
              <a className="text-blue-500 font-medium">View details</a>
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          borderRadius: isSender ? "16px 16px 0px 16px" : "16px 16px 16px 0px",
        }}
      >
